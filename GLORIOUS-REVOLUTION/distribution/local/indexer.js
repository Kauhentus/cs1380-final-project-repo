...
function start_index(callback) {
  ...
  const index_loop = async () => {
    try {
      ...
      console.error("Index Loop failed:", err);
  ...
  distribution.local.mem.get("links_to_index_map", (e1, links_to_index_map) => {
    distribution.local.mem.get("indexed_links_map", (e2, indexed_links_map) => {
      ...
      if (indexed_links_map.has(link)) {
        fs.appendFileSync(
          global.logging_path,
          `Link already indexed: ${link}\n`
        );
        return callback(null, { status: "skipped", reason: "already_indexed" });
      }

      links_to_index_map.set(link, true);
      fs.appendFileSync(global.logging_path, `Link added to index: ${link}\n`);
      callback(null, {
        status: "success",
        message: "Link added to index queue",
        link: link,
      });
    });
  });
  ...
  distribution.local.mem.get("links_to_index_map", (e1, links_to_index_map) => {
    distribution.local.mem.get("indexed_links_map", (e2, indexed_links_map) => {
      ...
      if (links_to_index_map.size === 0) {
        fs.appendFileSync(
          global.logging_path,
          `INDEXER SKIPPED BECAUSE NO LINKS AVAILABLE\n`
        );
      ...
      if (indexed_links_map.has(url)) {
        fs.appendFileSync(
          global.logging_path,
          `INDEXER SKIPPED BECAUSE ALREADY INDEXED: ${url}\n`
        );
        return callback(null, { status: "skipped", reason: "already_indexed" });
      }
      ...
      distribution.crawler_group.store.get(url, (e, v) => {
        ...
        distribution.local.groups.get("indexer_group", async (e, v) => {
          ...
          fs.appendFileSync(
            global.logging_path,
            `TOTAL INDEXING TIME: ${indexing_time}ms\n`
          );
