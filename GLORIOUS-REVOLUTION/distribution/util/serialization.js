...
function serialize(thing, seen = new Map(), isRoot = true) {
  ...
  if (thing === null) {
    ...
    if (seen.has(thing)) {
      object = { type: "reference", value: seen.get(thing) };
    } else {
      const uuid = uuidv4();
      seen.set(thing, uuid);
      object = {
        type: "array",
        id: uuid,
        value: thing.map(val => serialize(val, seen, false))
      };
      seen.delete(thing);
    }
    ...
    object = { type: "date", value: thing.toISOString() };
    ...
    if (seen.has(thing)) {
      ...
      const vals = keys.map(key => serialize(thing[key], seen, false));
      object = { type: "object", id: uuid, keys, vals };
      seen.delete(thing);
    }
    
  } else {
    throw new Error("Unsupported type: " + typeof thing);
  }

  // Only stringify once at the root call.
  return isRoot ? JSON.stringify(object) : object;
}
...
function deserialize(string, refs = {}) {
  ...
  throw new Error("did not match a valid structure type");
}
