...
function put(state, configuration, callback) {
  ...
  if (!fs.existsSync(node_store_path))
    fs.mkdirSync(node_store_path, { recursive: true });
...
function get(configuration, callback) {
  ...
  if (configuration === null) {
    ...
    return fs.readdir(
      path.join(node_store_path, configuration.gid),
      (err, all_keys) => {
        callback(null, all_keys);
      }
    );
  ...
  if (!fs.existsSync(file_path)) {
    callback(
      new Error(
        `local store get configuration not found (${JSON.stringify(
          configuration
        )}) (${file_path})`
      )
    );
...
function del(configuration, callback) {
  ...
  if (
    ...
    file_key = `${configuration.gid}-${configuration.key.replace(
      /[^a-zA-Z0-9_\-\.]/g,
      "_"
    )}`;
  } else {
...
function bulk_append(data, callback) {
  (async () => {
    ...
    try {
          ...
          new Promise(async (resolve, reject) => {
            ...
            for (const dataObj of groupedBatches[prefix]) {
              for (const term in dataObj) {
                ...
                mergedPrefixData[term] = mergedPrefixData[term].concat(
                  dataObj[term]
                );
              }
            }
            ...
            const filePath = path.join(
              groupDir,
              sanitizeKey(`prefix-${prefix}`) + ".json"
            );
            ...
            if (file_exists) {
              try {
                ...
                console.error(
                  `Error reading prefix data for ${prefix}: ${error.message}`
                );
              }
            }
      ...
      if (!global.logging_path) {
        // Create a default path or skip logging
        const dir = path.join(process.cwd(), "crawler-files", "logging");
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        global.logging_path = path.join(
          dir,
          `LOG-${global.nodeConfig.port}.txt`
        );
        console.log(`Created default logging path: ${global.logging_path}`);
      }
...
function read_bulk(configuration, callback) {
  (async () => {
    ...
    if (!("key" in configuration) || !("gid" in configuration))
      throw Error("bulk read requires key and gid");
