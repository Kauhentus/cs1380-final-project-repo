...
const scraped_urls = fs
  .readFileSync("./store_old/urls_scraped.txt")
  .toString()
  .split("\n");
const scraped_map = new Map<string, boolean>();
const to_scrap_urls = [
  ...new Set(
    fs.readFileSync("./store_old/urls_to_scrape.txt").toString().split("\n")
  ),
];
  ...
  const new_links = await fetch(`https://en.wikipedia.org${url}`)
    ...
    .then((html) => {
      ...
      const is_plant = hierarchy?.find(
        (pair) =>
          pair[0] === "kingdom" &&
          (pair[1].includes("plantae") || pair[1].includes("fungi"))
      );
      ...
      if (is_species_page) {
        ...
        fs.writeFileSync(
          `./store_old/plants/${binomial_name
            .replace(/\s+/g, "_")
            .replace(/[^a-z._]/g, "")}.json`,
      ...
      console.log(
        `# urls scraped: ${scraped_map.size
          .toString()
          .padStart(8, " ")}, # left: ${to_scrape_map.size
          .toString()
          .padStart(8, " ")}, # plants scraped: ${num_plants
          .toString()
          .padStart(8, " ")}`
      );
...
const run = async () => {
  while (to_scrape_map.size > 0) {
    ...
    new_urls.forEach((url) => {
      if (!scraped_map.has(url) && !to_scrape_map.has(url))
        to_scrape_map.set(url, true);
    });
...
setInterval(() => {
  fs.writeFileSync(
    "./store_old/urls_to_scrape.txt",
    Array.from(to_scrape_map.keys()).join("\n")
  );
}, 5000);
setInterval(() => {
  fs.writeFileSync(
    "./store_old/urls_scraped.txt",
    Array.from(scraped_map.keys()).join("\n")
  );
}, 5000);
